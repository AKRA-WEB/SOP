...
const server = http.createServer(async (req,res)=>{
  ...
  if (req.method==='POST' && new URL(req.url,'http://localhost').pathname==='/api') {
    ...
    } else res.end(JSON.stringify({status:'success',user:{roles:['ADMIN']},documents:[{...document,assets:document.assets.map(asset=>body.includeUrls===false ? {...asset,url:undefined} : asset)}]}));
    return;
  }
