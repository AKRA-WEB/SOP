...
- Load catalog metadata without waiting for Storage. Resolve only the opened guide's active files with authenticated batch signing; reuse expiring URLs in memory and refresh before expiry. SOP requests run beside the existing Sydney database.
