...
- Load catalog metadata without waiting for Storage. Resolve only the opened guide's active files with authenticated batch signing; reuse expiring URLs in memory and in this tab's sessionStorage after fresh catalog authorization. Cache matches the current user and guide revision/page metadata (at most20 entries); expired or changed entries are refetched. SOP requests run beside the existing Sydney database.
- Read private WebP copies (up to1600px/4MP, quality86); zoom and download retain the original image/PDF. New image uploads prepare an optional smaller copy, with original-only fallback where conversion is unsupported. Preload only the next image after the current page loads, respecting data-saving/2G connections.
